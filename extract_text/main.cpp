...
void write_on_file(const std::vector<std::pair<std::string, std::vector<std::pair<std::string, float>>>> data) {
    ...
    if(book)
    {
        Sheet *sheet = book->addSheet("WER result");
        for(int i = 0; i < data.size(); ++i) {
            if (sheet) {
                sheet->writeStr(1, i * fonts_count + 1, data[i].first.c_str());
                sheet->setMerge(1, 1, i * fonts_count + 1, fonts_count);
            }
        }
...
//    std::vector<std::pair<std::string, std::vector<std::string>>> all_dirs;
    ...
    sheet_i.push_back({"vgscvhd", 45});
    sheet_i.push_back({"vgscvhd", 45});
    sheet_i.push_back({"vgscvhd", 45});
    sheet_out.push_back({"cvdhcvdhcd", sheet_i});

    std::vector<std::pair<std::string, float>> sheet_ii;
    sheet_i.push_back({"vgscvhd", 45});
    sheet_i.push_back({"vgscvhd", 45});
    sheet_i.push_back({"vgscvhd", 45});
    sheet_out.push_back({"cvdhcvdhcd", sheet_ii});
...
//        sheet_out.push_back(std::make_pair(dir_i, sheet_it));
...
//                if(el_i.first == sheet_out[i].first) {
//                    sheet_out[i].second.push_back(std::make_pair(el_i.second.first, wer_val));
//                }
//            }
//
//            file_out.close();
